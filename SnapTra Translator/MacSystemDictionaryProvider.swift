...
final class MacSystemDictionaryProvider {
    func lookup(word: String, preferEnglish: Bool) -> DictionaryEntry? {
        ...
        #if DEBUG
        print("[MacSystemDictionaryProvider] System dictionary result for '\(word)' (\(html.count) chars):\n\(html.prefix(2000))")
        #endif

        if preferEnglish {
            return Self.parseEnglishHTML(html, word: word)
        }
        return Self.parseHTML(html, word: word)
    }
    ...
    private static func extractPartOfSpeechGroups(from html: String) -> [(String, String)] {
        ...
        for pattern in posPatterns {
            ...
            for (index, match) in matches.enumerated() {
                ...
                guard isValidPartOfSpeech(pos) else { continue }
    ...
    private static func normalizePOSLabel(_ label: String) -> String {
        ...
        let prefixes = [
            "transitive verb", "intransitive verb", "vt", "vi", "noun", "verb", "adjective", "adverb",
