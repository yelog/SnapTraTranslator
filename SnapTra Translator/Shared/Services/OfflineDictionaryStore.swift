...
/// Shared SQLite-backed storage for the offline ECDICT database.
final class OfflineDictionaryStore {
    ...
    func lookup(_ word: String) -> DictionaryEntry? {
        ...
        for variant in wordVariants(word) {
            if let entry = queryDatabase(variant) {
                return entry
            }
        }
        return nil
    ...
    private func queryDatabase(_ word: String) -> DictionaryEntry? {
        ...
        let definitions = buildDefinitions(translation: translation, definition: definition)
