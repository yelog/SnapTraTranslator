...
3. Add `ImageTranslationService` with Baidu V2 JSON request construction, language mapping, Access Token authentication, V2 response parsing, and a legacy signed multipart fallback for custom old endpoints.
4. Extend sentence service settings UI with a new image translation provider section.
5. Route paragraph and manual-region sentence translation through image translation when `Sentence Display` is set to `Image Translation`, resolve manual-region direction from a lightweight OCR pass when bidirectional translation is enabled, then display Baidu `paste_img` / `pasteImg` in-place over the source region.
